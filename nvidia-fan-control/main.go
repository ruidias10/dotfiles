...
func main() {
	...
	defer func() {
		...
		if ret != nvml.SUCCESS {
			...
			fmt.Printf("\nUnable to shutdown NVML: %v", nvml.ErrorString(ret))
		}
	}()
	...
	if ret != nvml.SUCCESS {
		...
		fmt.Printf("\nUnable to get device count: %v", nvml.ErrorString(ret))
	}
	...
	for {
		for i := 0; i < count; i++ {
			...
			if ret != nvml.SUCCESS {
				...
				fmt.Printf("\nUnable to get device at index %d: %v", i, nvml.ErrorString(ret))
				continue
			}
			...
			if ret != nvml.SUCCESS {
				...
				fmt.Printf("\nUnable to get temperature for GPU %d: %v", i, nvml.ErrorString(ret))
				continue
			}
			...
			fmt.Printf("\nUptime: %s Max Temp=%d°C Temp=%d°C, Speed=%d%%", elapsed, maxTemp, tempInt, newFanSpeed)

			output := fmt.Sprintf(
				"\nUptime:\t\t%s\nMax Temp:\t%d°C\nTemp:\t\t%d°C\nSpeed:\t\t%d%%\n",
				elapsed, maxTemp, tempInt, newFanSpeed)
			fmt.Print(output)
			...
			if newFanSpeed != prevFanSpeeds[i] {
				...
				if ret != nvml.SUCCESS {
					...
					fmt.Printf("\nUnable to set manual fan control policy for GPU %d: %v", i, nvml.ErrorString(ret))
					continue
				}
				...
				if ret != nvml.SUCCESS {
					...
					fmt.Printf("\nUnable to set fan speed for GPU %d: %v", i, nvml.ErrorString(ret))
					continue
				}

				// Set manual fan control policy
				ret = nvml.DeviceSetFanControlPolicy(device, 1, 1)
				if ret != nvml.SUCCESS {
					log.Printf("Unable to set manual fan control policy for GPU %d: %v", i, nvml.ErrorString(ret))
					fmt.Printf("\nUnable to set manual fan control policy for GPU %d: %v", i, nvml.ErrorString(ret))
					continue
				...
				// Set the new fan speed
				ret = nvml.DeviceSetFanSpeed_v2(device, 1, newFanSpeed)
				if ret != nvml.SUCCESS {
					log.Printf("Unable to set fan speed for GPU %d: %v", i, nvml.ErrorString(ret))
					fmt.Printf("\nUnable to set fan speed for GPU %d: %v", i, nvml.ErrorString(ret))
					continue
				...
				fmt.Printf("\nUpdated GPU %d: Temp=%d°C, Fan Speed=%d%%", i, tempInt, newFanSpeed)
